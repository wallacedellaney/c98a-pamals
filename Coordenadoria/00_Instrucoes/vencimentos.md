...
| BACO | tinha `.xlsx` na própria pasta de junho (`Controle_de_Vencimentos_JUN_2026(1).xlsx`) — formato padrão igual aos outros; o PDF de mesmo nome era só uma versão duplicada. **Incorporado.** |
...
| BACG | arquivo único (Diagonal+MAPEM+Vencimentos, unidade 1º/15º GAV, só a aeronave 2727, mês abril/2026). A seção "Controle de Vencimentos" em si usa a ordem de colunas **padrão**, só com AERONAVE sem prefixo "FAB" (igual à BAMN) — reaproveita `_processar_linhas` normal, só trocando o regex de matrícula. O `.ods` baixado do Drive veio com o `content.xml` **corrompido** (zlib não descomprime, ~100 bytes de payload) nas duas vezes que foi buscado via `download_file_content` — o mesmo arquivo veio íntegro via `read_file_content` (texto plano), usado pra reconstruir os dados manualmente num CSV local, validado linha a linha com `csv.reader` antes de salvar (mesma técnica do PAMA-LS). Tem muitos itens com DISPONIBILIDADE **"O/C"** (On Condition — sem vencimento programado por hora/pouso/calendário): tratado como um `tipo_vencimento` próprio, `"Condição"` (não é erro nem inconsistência, é uma categoria real de manutenção aeronáutica). **Incorporado** (tipo `csv_padrao_bare` no `REGISTRO`). |
